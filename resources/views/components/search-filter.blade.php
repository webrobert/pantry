<div class="block mt-4 flex items-center gap-3">
    <div class="search relative w-full">
        <x-jet-input wire:model="search" class="px-2.5 py-2 text-lg w-full shadow-md border-gray-700" placeholder="Search" />
        @if($search)
    ...
    <button x-data @click="$wire.emit('createItemFromSearch','{{ $search }}'), $wire.set('search', '')" {{-- added reset here may not like it --}}
            ...
            disabled:opacity-25 transition" style="padding: .675rem;">
